 ...
 */
function default_1(templateSource, branch, outputPath) {
    var _this = this;
    var spinner = (0, ora_1.default)("\u6B63\u5728\u4ECE ".concat(templateSource, " \u62C9\u53D6\u8FDC\u7A0B\u6A21\u677F...")).start();
    // 监听文件变化
    var watcher = chokidar_1.default.watch(outputPath, {
        persistent: true,
        ignoreInitial: true, // 不监听初始状态，只监听后续变化
    });
    // 监听新文件创建
    watcher.on('add', function (filePath) {
        var fileName = path.relative(outputPath, filePath);
        if (fileName.includes('.git'))
            return; // 忽略 .git 文件夹
        spinner.succeed(picocolors_1.default.bold("\u521B\u5EFA\u6587\u4EF6: ".concat(fileName)));
    });
